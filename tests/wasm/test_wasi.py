...
class TestMemoryAccess:
    ...
    def test_check_bounds_validates_offset(self) -> None:
        """Should validate memory offset bounds."""
        from notso_glb.wasm.wasi import WasiFilesystem

        fs = WasiFilesystem()
        fs._memory_array = bytearray(100)

        with pytest.raises(ValueError, match="negative offset"):
            fs._check_bounds("test", -1, 10)

    def test_check_bounds_validates_length(self) -> None:
        ...
        with pytest.raises(ValueError, match="out of bounds"):
            fs._check_bounds("test", 90, 20)

    def test_set_u8_writes_byte(self) -> None:
        """Should write single byte to memory."""
        from notso_glb.wasm.wasi import WasiFilesystem

        fs = WasiFilesystem()
        fs._memory_array = bytearray(10)

        fs._set_u8(5, 42)

        assert fs._memory_array[5] == 42

    def test_set_u8_masks_to_byte(self) -> None:
        ...
        assert fs._memory_array[5] == 0xFF

    def test_set_u32_writes_little_endian(self) -> None:
        ...
        fs._set_u32(0, 0x12345678)

        assert fs._memory_array[0] == 0x78
        assert fs._memory_array[1] == 0x56
        assert fs._memory_array[2] == 0x34
        assert fs._memory_array[3] == 0x12

    def test_get_u32_reads_little_endian(self) -> None:
        """Should read uint32 in little-endian format."""
        from notso_glb.wasm.wasi import WasiFilesystem

        fs = WasiFilesystem()
        fs._memory_array = bytearray([0x78, 0x56, 0x34, 0x12])

        value = fs._get_u32(0)

        assert value == 0x12345678

    def test_get_string_decodes_utf8(self) -> None:
        """Should decode UTF-8 string from memory."""
        from notso_glb.wasm.wasi import WasiFilesystem

        fs = WasiFilesystem()
        fs._memory_array = bytearray(b"Hello\x00World")

        text = fs._get_string(0, 5)
...
class TestWasiSyscalls:
    ...
    def test_fd_fdstat_get_identifies_output_fd(self) -> None:
        ...
        result = fs.wasi_fd_fdstat_get(1, 0)

        assert result == 0
        # Check filetype is character device (2)
        assert fs._memory_array[0] == 2

    def test_fd_fdstat_get_identifies_directory(self) -> None:
        ...
        result = fs.wasi_fd_fdstat_get(3, 0)

        assert result == 0
        # Check filetype is directory (3)
        assert fs._memory_array[0] == 3

    def test_fd_write_appends_to_output_buffer(self) -> None:
        ...
        fs._memory_array[0:5] = b"Hello"

        # Set up iovec: [ptr=0, len=5]
        fs._set_u32(10, 0)  # buffer ptr
        fs._set_u32(14, 5)  # buffer len

        result = fs.wasi_fd_write(1, 10, 1, 20)

        assert result == 0
        assert fs._output_buffer == b"Hello"
        assert fs._get_u32(20) == 5  # bytes written

    def test_fd_read_reads_from_file(self) -> None:
        ...
        assert result == 0
        assert bytes(fs._memory_array[50:55]) == b"Test "
        assert fs._get_u32(10) == 5  # bytes read
        assert fs._fds[10]["position"] == 5

    def test_fd_seek_updates_position(self) -> None:
        ...
        fs._init_fds()
        fs._memory_array = bytearray(100)
        fs._fds[10] = {"size": 100, "position": 0}

        result = fs.wasi_fd_seek32(10, 50, 0, 0)  # SEEK_SET

        assert result == 0
        assert fs._fds[10]["position"] == 50

    def test_fd_seek_seek_cur(self) -> None:
        ...
        fs._init_fds()
        fs._memory_array = bytearray(100)
        fs._fds[10] = {"size": 100, "position": 20}

        result = fs.wasi_fd_seek32(10, 10, 1, 0)  # SEEK_CUR

        assert result == 0
        assert fs._fds[10]["position"] == 30

    def test_fd_seek_seek_end(self) -> None:
        ...
        result = fs.wasi_fd_seek32(10, -10, 2, 0)  # SEEK_END

        assert result == 0
        assert fs._fds[10]["position"] == 90

    def test_fd_seek_validates_bounds(self) -> None:
        ...
        result = fs.wasi_fd_seek32(10, -10, 0, 0)  # Negative position
...
class TestEdgeCases:
    ...
    def test_fd_write_grows_file_buffer(self) -> None:
        ...
        assert result == 0
        assert len(fs._fds[10]["data"]) >= 50

    def test_fd_read_at_eof(self) -> None:
        ...
        fs._set_u32(0, 50)  # buffer ptr
        fs._set_u32(4, 10)  # buffer len

        result = fs.wasi_fd_read(10, 0, 1, 10)

        assert result == 0
        assert fs._get_u32(10) == 0  # 0 bytes read

    def test_handles_unicode_in_strings(self) -> None:
        ...
        fs._memory_array = bytearray(len(encoded))
        fs._memory_array[:] = encoded

        text = fs._get_string(0, len(encoded))
